...
export function tokenizeSentences(text: string): string[] {
  const normalized = text
    ...
    .replace(/(?<=[a-zäöüßy])\s+(?=(?:Die|Der|Das|Den|Dem|Alles|Dann|Danach|Zuerst|Zum|Zur|Schließlich|Nun|Anschließend|Zubereitung|Für|Mit|Dazu|Hierfür|Dabei|Sobald|Wenn|Während|Dafür|Unter|Auf|In|Aus|Bei|Preheat|Bake|Place|Add|Stir|Flake|Season|Mix|Pour|Cook|Heat)\b)/g, "\n")
